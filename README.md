...
- **同步字幕 + 实时滚动**：`.vtt` 解析（兼容常见内联标签与 SRT）。播放器内是**上一句 / 当前句 / 下一句的滚动窗口**（当前句高亮 + 进度条），右侧「轨迹」面板会**自动把当前句滚动到中间**；面板顶端可开关自动滚动，手动滚动时会暂时让位并浮出「回到当前」。
...
- **专辑文件夹管理（批量导入）**：一次可 **多选多个文件夹** 导入专辑库；若选中的父目录本身不含音频，会自动把它的子文件夹逐个作为专辑导入。点专辑卡片是预览（只读取曲目列表，点「载入并播放」才切换）；可设置专辑封面（整张专辑共用）或从列表移除（不删文件）。
...
- **专辑库**：左侧 **专辑** 视图汇总你的文件夹。右上角 **＋ 导入** 支持一次多选多个文件夹（选中的目录若不含音频，会改为导入其子文件夹）；点卡片是 **预览**（展开曲目列表），预览里的 **载入并播放** 才切换到该专辑。卡片右侧可 **设封面**（整张专辑共用）或 **从列表移除**（不会删除文件）。
