...
- **专辑文件夹管理**：把常用文件夹收藏成“专辑库”。**添加专辑、预览专辑都不会打断当前播放**（预览只读取曲目列表，点「载入并播放」才切换）；可设置专辑封面（整张专辑共用）或从列表移除（不删文件）。
...
- **专辑库**：左侧 **专辑** 视图汇总你的文件夹。点卡片是**预览**（展开曲目列表，当前播放继续，不被打断）；预览里的 **载入并播放** 才切换到该专辑。卡片右侧可 **设封面**（整张专辑共用）或 **从列表移除**（不会删除文件）。**添加** 也只登记到专辑库，不影响正在播放的内容。
